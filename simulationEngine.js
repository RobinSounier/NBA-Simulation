...
(function () {
  ...
  function createRoster(teamDef, usedNames) {
    ...
    positions.forEach((pos) => {
      ...
      if (teamDef.tier === 1 && pos === "SF" && Math.random() > 0.5)
        rating = randomRange(92, 98);
      if (teamDef.tier === 1 && pos === "PG" && Math.random() > 0.5)
        rating = randomRange(90, 96);
  ...
  function generateRegularSeasonSchedule(teams) {
    ...
    intraConfMatchups.forEach((m) => {
      if (
        currentCounts[m.home] < targetGames &&
        currentCounts[m.away] < targetGames
      ) {
        finalMatchups.push(m);
        currentCounts[m.home]++;
        currentCounts[m.away]++;
    ...
    for (let game of finalMatchups) {
      ...
      for (let limit = 4; limit <= 7; limit++) {
        for (let w = 0; w < 24; w++) {
          ...
          const homeCount = wGames.filter(
            (g) => g.home === game.home || g.away === game.home,
          ).length;
          const awayCount = wGames.filter(
            (g) => g.home === game.away || g.away === game.away,
          ).length;
          const alreadyPlayingEachOther = wGames.some(
            (g) =>
  ...
  function simulateSingleGame(game, teams, alerts) {
    ...
    const homeStartersRating =
      activeHomeStarters.reduce((acc, p) => acc + p.rating, 0) / 5;
    const awayStartersRating =
      activeAwayStarters.reduce((acc, p) => acc + p.rating, 0) / 5;

    // Calculer la fatigue moyenne des titulaires actifs
    const homeStartersFatigue =
      activeHomeStarters.reduce((acc, p) => acc + p.fatigue, 0) / 5;
    const awayStartersFatigue =
      activeAwayStarters.reduce((acc, p) => acc + p.fatigue, 0) / 5;

    // Calculer le moral moyen des titulaires actifs
    const homeStartersMoral =
      activeHomeStarters.reduce((acc, p) => acc + p.moral, 0) / 5;
    const awayStartersMoral =
      activeAwayStarters.reduce((acc, p) => acc + p.moral, 0) / 5;
    ...
    const simulateScore = (
      ...
      const scoreFactor =
        ratingContribution + randomContribution + terrainContribution;
    ...
    const simulateIndividualStats = (
      ...
      const activeBench = healthyPlayers.filter(
        (p) => !activeStarters.includes(p),
      );
      ...
      if (activeBench.length > 0) {
        ...
        activeBench.forEach((p, idx) => {
          minutesAllocation[p.id] =
            idx === activeBench.length - 1 ? remainingMin - share * idx : share;
        });
      }
      ...
      const statsWeights = teamRoster.map((p) => {
        if (p.injury)
          return { id: p.id, ptsW: 0, rebW: 0, astW: 0, stlW: 0, blkW: 0 };
        ...
        return {
          ...
          blkW:
            min *
            (p.position === "C" || p.position === "PF" ? 2.5 : 0.5) *
            ratingFactor,
        };
      });
      ...
      teamRoster.forEach((p) => {
        ...
        if (mins > 0) {
          ...
          const injuryProb =
            p.fatigue > 85 ? 0.1 : isStarterActive ? 0.02 : 0.005;

          // Compter le nombre de blessures en cours de l'équipe (max 2)
          const currentInjuriesCount = teamRoster.filter(
            (pl) => pl.injury,
          ).length;
    ...
    const homePlayerStats = simulateIndividualStats(
      homeTeam,
      homeScore,
      awayScore,
      activeHomeStarters,
    );
    const awayPlayerStats = simulateIndividualStats(
      awayTeam,
      awayScore,
      homeScore,
      activeAwayStarters,
    ...
    Object.values(homePlayerStats).forEach((ps) => {
      if (ps.pts > topScorer.pts)
        topScorer = {
          name: ps.name,
          pts: ps.pts,
          team: homeTeam.city + " " + homeTeam.name,
        };
    });
    Object.values(awayPlayerStats).forEach((ps) => {
      if (ps.pts > topScorer.pts)
        topScorer = {
          name: ps.name,
          pts: ps.pts,
          team: awayTeam.city + " " + awayTeam.name,
        };
    });
    ...
    if (alerts.length > 0) {
      const injuryAlert = alerts.find(
        (a) =>
          a.type === "injury" &&
          (a.team.includes(homeTeam.name) || a.team.includes(awayTeam.name)),
      );
      if (injuryAlert) {
  ...
  function assignPositionsAndRosters(teams, playersData) {
    ...
    Object.keys(teams).forEach((teamName) => {
      ...
      rosterPlayers.forEach((p, idx) => {
        ...
        const score =
          (p.standingDunk || 30) + (p.defensiveRebound || 50) + (p.block || 30);
        if (score > maxCScore) {
          maxCScore = score;
          cCandidate = idx;
        }
  ...
  function calculateSalary(rating, starter) {
  ...
  function checkPlayerPromises(state, alerts) {
    ...
    userTeamObj.roster.forEach((p) => {
      if (
        p.promisedWeeksRemaining !== undefined &&
        p.promisedWeeksRemaining > 0
      ) {
  ...
  function generateWeeklyEmails(state, alerts) {
    ...
    if (type === "trade" && userTeamObj.roster.length > 0) {
      ...
      const otherTeamNames = Object.keys(state.teams).filter(
        (t) => t !== state.userTeam,
      );
      ...
      const matchingPlayers = otherTeamObj.roster.filter(
        (p) =>
          Math.abs(p.rating - userPlayer.rating) <= 4 &&
          p.id !== userPlayer.id &&
          !p.starter,
      );
      ...
      if (
        ...
        const otherTeams = Object.keys(state.teams).filter(
          (t) => t !== state.userTeam,
        );
      ...
      const benchPlayers = userTeamObj.roster.filter(
        (p) => !p.starter && !p.injury && p.rating > 70,
      );
        ...
        switch (action) {
          case "init_season": {
            ...
            if (params.playersData && Array.isArray(params.playersData)) {
              ...
              Object.keys(teams).forEach((tName) => {
                ...
                const def = TEAM_DEFS.find(
                  (d) => d.city + " " + d.name === tName,
                );
                teamObj.roster = createRoster(def, usedNames);
              });
            }

            // Calculer le salaire utilisé de chaque équipe
            Object.values(teams).forEach((team) => {
            ...
            const owner = userTeamName.includes("Lakers")
              ? "Jeanie Buss (Propriétaire)"
              : userTeamName.includes("Celtics")
                ? "Wyc Grousbeck (Propriétaire)"
                : "La Propriété de la Franchise";
            state.inbox = [
              {
                id: "welcome_email",
                sender: owner,
          ...
          case "simulate_game": {
            if (state.stage !== "regular_season") {
              throw new Error(
                "L'action simulate_game n'est disponible qu'en saison régulière.",
              );
            }
            ...
            if (params && params.gameId) {
              gameToSimulate = state.schedule.find(
                (g) => g.id === params.gameId && !g.simulated,
              );
            } else {
          ...
          case "simulate_week": {
            if (state.stage !== "regular_season") {
              throw new Error(
                "L'action simulate_week n'est disponible qu'en saison régulière.",
              );
            }

            const currentWeek = state.currentWeek;
            const weekGames = state.schedule.filter(
              (g) => g.week === currentWeek && !g.simulated,
            );
            ...
            if (weekGames.length === 0) {
              ...
              if (state.currentWeek > 24) {
                ...
                narrative =
                  "La saison régulière est terminée ! Les playoffs vont débuter.";
              } else {
            ...
            weekGames.forEach((game) => {
              ...
              gamesCount++;
              if (
                game.home === state.userTeam ||
                game.away === state.userTeam
              ) {
                userGameResult = game;
            ...
            if (userGameResult) {
              ...
              const userWon =
                (userGameResult.home === state.userTeam && uHome > uAway) ||
          ...
          case "set_lineup": {
            ...
            lineup.forEach((pId) => {
              if (!rosterIds.includes(pId)) {
                throw new Error(
                  `Le joueur "${pId}" n'appartient pas à cette équipe.`,
                );
              }
              const player = team.roster.find((p) => p.id === pId);
              if (player.injury && player.injury.severity === "grave") {
                throw new Error(
                  `Le joueur ${player.name} est gravement blessé et ne peut pas être aligné.`,
                );
              }
            });
          ...
          case "resolve_email": {
            ...
            if (accept) {
              ...
              if (email.type === "trade") {
                const { userPlayerId, offeredPlayerId, otherTeam } =
                  email.actionData;
                const otherTeamObj = state.teams[otherTeam];

                const userPlayerIdx = userTeamObj.roster.findIndex(
                  (p) => p.id === userPlayerId,
                );
                const offeredPlayerIdx = otherTeamObj.roster.findIndex(
                  (p) => p.id === offeredPlayerId,
                );

                if (userPlayerIdx === -1 || offeredPlayerIdx === -1) {
                  throw new Error(
                    "Les joueurs concernés par l'échange sont introuvables.",
                  );
                }
                ...
                const faIdx = state.freeAgents.findIndex(
                  (p) => p.id === faPlayerId,
                );
                if (faIdx === -1) {
                  throw new Error(
                    "L'agent libre proposé n'est plus disponible.",
                  );
                }
                ...
                if (userTeamObj.roster.length >= 15) {
                  ...
                  const cutIdx = userTeamObj.roster.findIndex(
                    (p) => p.id === playerToCut.id,
                  );
                ...
                const player = userTeamObj.roster.find(
                  (p) => p.id === playerId,
                );
                if (player) {
          ...
          case "sign_free_agent": {
            const { faPlayerId } = params;
            if (!faPlayerId) {
              throw new Error("L'identifiant de l'agent libre est requis.");
            }
            ...
            const faIndex = state.freeAgents.findIndex(
              (p) => p.id === faPlayerId,
            );
            if (faIndex === -1) {
              throw new Error("Cet agent libre n'existe pas.");
            }
            ...
            if (freeAgent.salary > availableSalary) {
              throw new Error(
                `Espace salarial insuffisant. Besoin: ${Math.round(freeAgent.salary / 1000000)}M$, Disponible: ${Math.round(availableSalary / 1000000)}M$.`,
              );
            }

            // Add to roster
            freeAgent.starter = false;
            userTeamObj.roster.push(freeAgent);
            userTeamObj.used_salary += freeAgent.salary;
            ...
            alerts.push({
              team: state.userTeam,
              type: "signing",
              detail: `SIGNATURE EFFECTUÉE : ${freeAgent.name} (OVR ${freeAgent.rating}) a signé un contrat de ${Math.round(freeAgent.salary / 1000000)}M$ avec votre équipe !`,
            });

            narrative = `${freeAgent.name} a rejoint l'effectif des ${state.userTeam} pour un salaire de ${Math.round(freeAgent.salary / 1000000)}M$.`;
            data = {
              success: true,
              newRosterSize: userTeamObj.roster.length,
              usedSalary: userTeamObj.used_salary,
            };
            break;
          }
          ...
          case "release_player": {
            ...
            const playerIdx = userTeamObj.roster.findIndex(
              (p) => p.id === playerId,
            );
            if (playerIdx === -1) {
              throw new Error(
                "Ce joueur ne fait pas partie de votre effectif.",
              );
            }

            if (userTeamObj.roster.length <= 10) {
              throw new Error(
                "Votre effectif doit comporter au moins 10 joueurs. Recrutez d'abord d'autres agents libres !",
              );
            }
          ...
          case "advance_playoffs": {
            ...
            if (state.stage === "regular_season") {
              ...
              if (remaining > 0) {
                throw new Error(
                  `Impossible de lancer les playoffs : il reste ${remaining} matchs de saison régulière.`,
                );
              }
            }
            ...
            if (state.stage === "playoffs_init") {
              ...
              const sortFn = (a, b) =>
                b.pct !== a.pct ? b.pct - a.pct : b.pointsDiff - a.pointsDiff;
              standings.East.sort(sortFn);
              standings.West.sort(sortFn);

              const eastQualifiers = standings.East.slice(0, 8).map(
                (t) => t.name,
              );
              const westQualifiers = standings.West.slice(0, 8).map(
                (t) => t.name,
              );
              ...
              narrative =
                "Les Playoffs NBA commencent ! Les 16 meilleures équipes se disputent le titre suprême.";
            ...
            if (state.stage !== "playoffs") {
              throw new Error(
                "L'action advance_playoffs n'est disponible qu'en phase de playoffs.",
              );
            }
            ...
            const roundFinished = activeSeriesList.every(
              (s) => s.w1 === 4 || s.w2 === 4,
            );
            ...
            if (roundFinished) {
              ...
              if (currentRound === 1) {
                ...
                playoffs.series.East = [
                  {
                    t1: getWinner(playoffs.series.East[0]),
                    t2: getWinner(playoffs.series.East[1]),
                    w1: 0,
                    w2: 0,
                    games: [],
                  },
                  {
                    t1: getWinner(playoffs.series.East[2]),
                    t2: getWinner(playoffs.series.East[3]),
                    w1: 0,
                    w2: 0,
                    games: [],
                  },
                ...
                playoffs.series.West = [
                  {
                    t1: getWinner(playoffs.series.West[0]),
                    t2: getWinner(playoffs.series.West[1]),
                    w1: 0,
                    w2: 0,
                    games: [],
                  },
                  {
                    t1: getWinner(playoffs.series.West[2]),
                    t2: getWinner(playoffs.series.West[3]),
                    w1: 0,
                    w2: 0,
                    games: [],
                  },
                ...
                playoffs.series.Finals = [
                  {
                    t1: getWinner(playoffs.series.East[0]),
                    t2: getWinner(playoffs.series.West[0]),
                    w1: 0,
                    w2: 0,
                    games: [],
                  },
                ...
                narrative =
                  "Finales NBA lancées ! Le titre se joue maintenant.";
              } else if (currentRound === 4) {
            ...
            activeSeriesList.forEach((series) => {
              if (series.w1 < 4 && series.w2 < 4) {
                ...
                const wonT1 =
                  (gameObj.home === series.t1 &&
                    gameObj.result.homeScore > gameObj.result.awayScore) ||
                  (gameObj.away === series.t1 &&
                    gameObj.result.awayScore > gameObj.result.homeScore);
                ...
                if (
                  series.t1 === state.userTeam ||
                  series.t2 === state.userTeam
                ) {
                  userGameSimulated = gameRecord;
                  narrative =
                    res.narrative +
                    ` (Série : ${series.t1} ${series.w1} - ${series.w2} ${series.t2})`;
                }
              }
            });
          ...
          case "save_live_game_result": {
            ...
            if (isPlayoff) {
              ...
              const series = activeSeriesList.find(
                (s) => s.t1 === state.userTeam || s.t2 === state.userTeam,
              );
              if (!series)
                throw new Error(
                  "Série de playoffs introuvable pour l'utilisateur.",
                );
              ...
              if (!gameObj)
                throw new Error("Match de saison régulière introuvable.");
            ...
            liveAlerts.forEach((la) => {
              ...
              if (team) {
                ...
                if (player) {
                  const severity = la.detail.includes("sévérité: Légère")
                    ? "Légère"
                    : la.detail.includes("sévérité: Grave")
                      ? "Grave"
                      : "Moyenne";
                  let days = 7;
