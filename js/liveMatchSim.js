(function() {
  ...
  const INJURY_TYPES = [
    { name: "Entorse de la cheville", severity: "Moyenne", minDays: 5, maxDays: 14 },
    { name: "Douleur au genou", severity: "Légère", minDays: 2, maxDays: 6 },
    { name: "Contracture musculaire", severity: "Légère", minDays: 3, maxDays: 8 },
    { name: "Déchirure des ligaments", severity: "Grave", minDays: 30, maxDays: 90 },
    { name: "Fracture du doigt", severity: "Moyenne", minDays: 12, maxDays: 24 }
  ];
  ...
  window.LiveMatchSim = {
    ...
    stepLiveMatch: function(matchState) {
      ...
      const possessionTime = Math.min(timeRemaining, Math.floor(Math.random() * 8) + 10); // 10 à 18 secondes (moyenne 14s)
      ...
      const getSubstitutes = (team, activeOnCourt) => {
        ...
        if (useBench && healthyBench.length > 0) {
          ...
          healthyStarters.forEach(starter => {
            let backup = healthyBench.find(b => b.position === starter.position && !usedBenchIds.has(b.id));
            if (!backup) {
          ...
          while (court.length < 5 && healthyStarters.some(s => !court.includes(s))) {
          ...
          if (court.length < 5) {
            const healthyBenchSorted = healthyBench.sort((a, b) => b.rating - a.rating);
      ...
      const getNamesList = (players) => players.map(p => p.name).sort().join(", ");
      ...
      const nextHomeStarters = getSubstitutes(matchState.homeTeam, matchState.activeHomeStarters);
      const nextAwayStarters = getSubstitutes(matchState.awayTeam, matchState.activeAwayStarters);
      ...
      addMinutes(matchState.homeStats, matchState.activeHomeStarters, possessionTime);
      addMinutes(matchState.awayStats, matchState.activeAwayStarters, possessionTime);
      ...
      const attackingStats = matchState.possession === "home" ? matchState.homeStats : matchState.awayStats;
      const defendingStats = matchState.possession === "home" ? matchState.awayStats : matchState.homeStats;
      const attackingStarters = matchState.possession === "home" ? matchState.activeHomeStarters : matchState.activeAwayStarters;
      const defendingStarters = matchState.possession === "home" ? matchState.activeAwayStarters : matchState.activeHomeStarters;

      // Moyennes des ratings
      const avgAttRating = attackingStarters.reduce((acc, p) => acc + p.rating, 0) / 5;
      const avgDefRating = defendingStarters.reduce((acc, p) => acc + p.rating, 0) / 5;

      // Blessure aléatoire extrêmement rare (0.04% chance par possession)
      if (Math.random() < 0.0004) {
        const teamObj = Math.random() > 0.5 ? matchState.homeTeam : matchState.awayTeam;
        const roster = teamObj.roster;
        const healthyOnCourt = teamObj === matchState.homeTeam ? matchState.activeHomeStarters : matchState.activeAwayStarters;
        
        if (healthyOnCourt.length > 0) {
          const injuredPlayer = healthyOnCourt[Math.floor(Math.random() * healthyOnCourt.length)];
          const injuryDef = INJURY_TYPES[Math.floor(Math.random() * INJURY_TYPES.length)];
          const duration = Math.floor(Math.random() * (injuryDef.maxDays - injuryDef.minDays + 1)) + injuryDef.minDays;
          ...
          const bench = roster.filter(p => !p.starter && !p.injury && !healthyOnCourt.includes(p))
            .sort((a, b) => b.rating - a.rating);

          if (teamObj === matchState.homeTeam) {
            matchState.activeHomeStarters = matchState.activeHomeStarters.filter(p => p.id !== injuredPlayer.id);
            if (bench.length > 0) matchState.activeHomeStarters.push(bench[0]);
            matchState.homeStats[injuredPlayer.id].injury = true;
          } else {
            matchState.activeAwayStarters = matchState.activeAwayStarters.filter(p => p.id !== injuredPlayer.id);
          ...
          const template = COMMENTARY_TEMPLATES.injury[Math.floor(Math.random() * COMMENTARY_TEMPLATES.injury.length)];
          matchState.commentaries.push({
      ...
      const attWeights = attackingStarters.map(p => ({
        player: p,
        weight: p.rating * (p.position === "PG" || p.position === "SG" ? 1.3 : 1.0)
      }));
      ...
      const selectedDefender = defendingStarters[Math.floor(Math.random() * defendingStarters.length)];
      ...
      if (playRoll < 0.08) {
        ...
        if (turnoverRoll < 0.40) {
          // Interception
          defendingStats[selectedDefender.id].stl++;
          const txt = getStealString(selectedDefender.name);
          matchState.commentaries.push({ text: txt, type: "steal", time: clockStr, q: q });
        } else {
          // Perte de balle simple
          const txt = getTurnoverString(selectedAttacker.name);
          matchState.commentaries.push({ text: txt, type: "turnover", time: clockStr, q: q });
        }
        matchState.possession = matchState.possession === "home" ? "away" : "home";
        ...
        matchState.commentaries.push({ text: foulTxt, type: "foul", time: clockStr, q: q });

        const ftPct = 0.70 + (selectedAttacker.rating - 75) * 0.005;
        const ftCount = 2;
        let ftMade = 0;
        ...
        for (let i = 0; i < ftCount; i++) {
          ...
          if (ftRoll) {
            ...
            const ftTxt = getFreeThrowMadeString(selectedAttacker.name) + ` (${i+1}/${ftCount})`;
            matchState.commentaries.push({ text: ftTxt, type: "score-success", time: clockStr, q: q });
          } else {
            const ftTxt = getFreeThrowMissedString(selectedAttacker.name) + ` (${i+1}/${ftCount})`;
            matchState.commentaries.push({ text: ftTxt, type: "miss", time: clockStr, q: q });
          }
        }

        // Rebond si le dernier LF est raté
        if (ftMade < ftCount && Math.random() > 0.5) {
          handleRebound(matchState, defendingStarters, attackingStarters, defendingStats, attackingStats, clockStr, q);
        } else {
          matchState.possession = matchState.possession === "home" ? "away" : "home";
        }
        ...
        // TIR À 3 POINTS
        const threeProb = 0.33 + (selectedAttacker.rating - 75) * 0.006 + (avgAttRating - avgDefRating) * 0.003;
        ...
        if (isMade) {
          ...
          const txt = getThreePointerMadeString(selectedAttacker.name) + assistStr;
          matchState.commentaries.push({ text: txt, type: "score-success", time: clockStr, q: q });
          matchState.possession = matchState.possession === "home" ? "away" : "home";
        } else {
          const txt = getThreePointerMissedString(selectedAttacker.name);
          matchState.commentaries.push({ text: txt, type: "miss", time: clockStr, q: q });
          handleRebound(matchState, defendingStarters, attackingStarters, defendingStats, attackingStats, clockStr, q);
        }
        ...
        const isBlocked = (selectedDefender.position === "C" || selectedDefender.position === "PF") && Math.random() < blockProb;

        if (isBlocked) {
          defendingStats[selectedDefender.id].blk++;
          const blockTxt = getBlockString(selectedDefender.name);
          matchState.commentaries.push({ text: blockTxt, type: "block", time: clockStr, q: q });
          handleRebound(matchState, defendingStarters, attackingStarters, defendingStats, attackingStats, clockStr, q);
        } else {
          const twoProb = 0.45 + (selectedAttacker.rating - 75) * 0.006 + (avgAttRating - avgDefRating) * 0.003;
          ...
          if (isMade) {
            ...
            const txt = (isDunk ? getDunkMadeString(selectedAttacker.name) : getTwoPointerMadeString(selectedAttacker.name)) + assistStr;
            matchState.commentaries.push({ text: txt, type: "score-success", time: clockStr, q: q });
            matchState.possession = matchState.possession === "home" ? "away" : "home";
          } else {
      ...
      if (matchState.quarterTime <= 0) {
        ...
        matchState.commentaries.push({ text: quarterEndText, type: "quarter-end", time: "0:00", q: q });
        ...
        if (q < 4) {
          ...
          if (matchState.homeScore === matchState.awayScore) {
            ...
            matchState.possession = Math.random() > 0.5 ? "home" : "away";
            matchState.commentaries.push({
              text: COMMENTARY_TEMPLATES.ot_start[Math.floor(Math.random() * COMMENTARY_TEMPLATES.ot_start.length)],
              type: "news",
              time: "5:00",
  ...
  // Rebondeur
  function handleRebound(matchState, defStarters, attStarters, defStats, attStats, clockStr, q) {
    ...
    if (isDef) {
      ...
      matchState.commentaries.push({ text: txt, type: "rebound", time: clockStr, q: q });
      matchState.possession = matchState.possession === "home" ? "away" : "home";
    } else {
  ...
  function compileBoxScore(matchState) {
    const buildPlayerStatsFormat = (stats, team) => {
      ...
      Object.keys(stats).forEach(id => {
        ...
        const plusMinus = matchState.homeScore > matchState.awayScore ? 
          (team === matchState.homeTeam ? Math.floor(Math.random()*10)+2 : -Math.floor(Math.random()*10)-2) : 
          (team === matchState.awayTeam ? Math.floor(Math.random()*10)+2 : -Math.floor(Math.random()*10)-2);
    ...
    matchState.result = {
      ...
      homePlayerStats: buildPlayerStatsFormat(matchState.homeStats, matchState.homeTeam),
      awayPlayerStats: buildPlayerStatsFormat(matchState.awayStats, matchState.awayTeam)
    };
  }

  // Tiers de templates
  const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
  function getThreePointerMadeString(name) { return pick(COMMENTARY_TEMPLATES.three_pointer_made).replace("{player}", name); }
  function getThreePointerMissedString(name) { return pick(COMMENTARY_TEMPLATES.three_pointer_missed).replace("{player}", name); }
  function getTwoPointerMadeString(name) { return pick(COMMENTARY_TEMPLATES.two_pointer_made).replace("{player}", name); }
  function getTwoPointerMissedString(name) { return pick(COMMENTARY_TEMPLATES.two_pointer_missed).replace("{player}", name); }
  function getDunkMadeString(name) { return pick(COMMENTARY_TEMPLATES.dunk_made).replace("{player}", name); }
  function getAssistString(name) { return pick(COMMENTARY_TEMPLATES.assist).replace("{passer}", name); }
  function getDefReboundString(name) { return pick(COMMENTARY_TEMPLATES.def_rebound).replace("{player}", name); }
  function getOffReboundString(name) { return pick(COMMENTARY_TEMPLATES.off_rebound).replace("{player}", name); }
  function getBlockString(name) { return pick(COMMENTARY_TEMPLATES.block).replace("{player}", name); }
  function getStealString(name) { return pick(COMMENTARY_TEMPLATES.steal).replace("{player}", name); }
  function getTurnoverString(name) { return pick(COMMENTARY_TEMPLATES.turnover).replace("{player}", name); }
  function getFoulString(name) { return pick(COMMENTARY_TEMPLATES.foul).replace("{player}", name); }
  function getFreeThrowMadeString(name) { return pick(COMMENTARY_TEMPLATES.free_throw_made).replace("{player}", name); }
  function getFreeThrowMissedString(name) { return pick(COMMENTARY_TEMPLATES.free_throw_missed).replace("{player}", name); }
